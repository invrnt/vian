...
test('auth commands expose only safe metadata and fail closed for login', async () => {
  ...
  try {
    ...
    expect(await authCommand.run(['login', 'openai-chatgpt'], context)).toBe(1);
    expect(stderr.join('')).toContain('supported Vian OAuth client arrangement');
