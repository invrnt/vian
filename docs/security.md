...
Bot `.env` values are resolved for that bot only. ChatGPT OAuth profiles live in the user's Vian credential directory with restrictive permissions and may be shared by several bots. Do not copy profile JSON into bot directories or logs. `vian inspect` prints references, not secret values.
