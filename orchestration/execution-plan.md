...
| [C: local-cli](agent-prompts/local-cli.md) | Non-destructive init, registry and offline inspection/history commands | S integrated; C01,C03,C06,C07 stable | `packages/cli/src/local/` except transferred online doctor paths; `packages/cli/templates/`; remaining colocated tests | V003,V007; T02 | 2–3 d, medium | O; fake vertical slice |
...
| [P: providers](agent-prompts/providers.md) | Google/Gateway and subscription OAuth adapters, credential ownership and auth CLI | F integrated; C01,C05,C06 stable; pinned source-level OAuth/transport evidence available, live compatibility pending | `packages/provider-google/, packages/provider-vercel/, packages/provider-openai-chatgpt/, packages/credentials/ except manifests`; `packages/cli/src/auth/` | V026–V030; T05 | 3–6 d, low | O |
...
| [O: operations](agent-prompts/operations.md) | Assemble live product, operational CLI, services and documentation; verify first fake slice before live wiring | C,T,R,P,G integrated; C01–C09 stable; required product decisions resolved | `packages/cli/src/main.ts and loader after F`; `packages/cli/src/operations/`; `packages/cli/src/local/doctor.ts` and online-doctor tests after C transfer; `packages/runtime/ and tests/integration/ after R`; `docs/`; `README.md`; `tests/e2e/`; `benchmarks/` | V008,V009,V037–V042; T03,T07 | 2–4 d, medium-low | H |
...
4. C → O: `packages/cli/src/local/doctor.ts` and online-doctor tests (including a new colocated `doctor.online.test.ts`) for required `doctor --online` only. C implementation was integrated by verified revision `29978a4`; C explicitly acknowledged this transfer and stopped editing these paths. Other `packages/cli/src/local/` paths and tests remain C-owned until H.
5. All owners → H: F/S/C/T/R/P/G/O implementation handoffs were integrated and verified by source revision `b2523fa`; all former owners completed and stopped editing their paths, and O explicitly acknowledged this transfer at that revision. H now owns all implementation paths for bounded release fixes, including migrations, generated files and CI, and `orchestration/release-report.md`. `orchestration/execution-state.md` remains coordinator-only. Former owners may advise but may not edit unless H records a further transfer.
6. Post-release Telegram verification assignment: the coordinator transferred `packages/core/src/storage.ts` exclusively to the `telegram_verify` implementation agent from verified `main` revision `1e0c312` on 2026-09-24. No other active assignment edits that contract file. The agent must acknowledge this transfer before editing it; the coordinator integrates its committed branch after validation.
...
F, S, C, T, R, P and G are integrated; O is running and H follows O. The implemented P subscription path uses the user-authorized, MIT-licensed OpenClaw source at commit `0e79899fedc26ae9a90a196a8bc41b97fd39d7e5`. Combined offline verification at `183249d` passed 97 tests, build and compiled CLI smoke. This removes the earlier source-level implementation blocker but does not establish live Vian login, inference, refresh or multi-bot compatibility. Official OpenAI documentation alone does not document this third-party contract. Product evidence and remaining live requirements are in [providers.md](product/providers.md).

Live release checks still require authorized subscription, Gateway, Gemini and Telegram test accounts plus a fresh Linux service environment. Those checks remain blocked until access is available; no fixture result may be labeled live. User decisions on ambiguous delivery, group controls, default steering and reset ordering remain resolved in [identity-history.md](product/identity-history.md). C09 SDK continuation compatibility was proven by F and R offline. O may continue independent assembly and H must report any unverified gate explicitly. No estimate includes waiting for external authorization.
