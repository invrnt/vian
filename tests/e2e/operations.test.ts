...
test('one broken bot stays in error while another starts; failed restart keeps the healthy instance', async () => {
  ...
  const daemon = new VianDaemon({
    assemble: async record => {
      ...
      return { close: async () => { closes++; } } as never;
    },
  ...
  try {
    ...
    if (response.ok) expect(response.data).toEqual(expect.arrayContaining([{ bot: 'healthy', id: expect.any(String), enabled: true, status: 'running' }, { bot: 'broken', id: expect.any(String), enabled: true, status: 'error', error: 'Invalid bot config' }]));
